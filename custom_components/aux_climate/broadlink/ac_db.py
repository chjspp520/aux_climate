...
# Raw payload log file path (next to this source file)
_RAW_LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ac_raw_log.txt")

def log_raw_payload(label, payload):
    """Append raw decrypted payload (hex + decimal) to log file for analysis."""
    try:
...
class ac_db(device):
	...
	def get_ac_status(self,force_update = False):
		##Check if the status is up to date to reduce timeout issues
		self.logger.debug("Last update was: %s" % self.status['lastupdate'] )
		if (force_update == False and 
			self.status['lastupdate'] is not None and 
			(self.status['lastupdate'] + self.update_interval) > time.time()):
			return self.make_nice_status(self.status)
		
		##Non-blocking lock: only one caller actually queries the device
		##Others return cached status immediately instead of all timing out
		if not self._update_lock.acquire(blocking=False):
			self.logger.debug("Update already in progress, returning cached status")
			return self.make_nice_status(self.status)
		...
		try:
			...
			if status:
				self.status['lastupdate'] = time.time()
			return status if status else self.make_nice_status(self.status)
		finally:
	...
	def __init__ (self, host, mac,name=None,cloud=None,debug = False,update_interval = 0,devtype=None,bind_to_ip=None):			
		...
		if self.auth() == False:
			...
			raise Exception("Authentication Failed to AC")
	...
	def set_default_values(self):
		...
		self.status['room_hum'] = None
		self.status['err_code'] = 0
	...
	def set_mute(self,value):
		...
		if isinstance(value, bool):
			mode = self.STATIC.ONOFF.ON if value else self.STATIC.ONOFF.OFF
		else:
	...
	def get_raw_logging(self):
		"""Return whether raw payload logging is enabled."""
		return self._raw_log_enabled

	def set_raw_logging(self, enabled: bool):
		"""Enable or disable raw payload logging to ac_raw_log.txt."""
		self._raw_log_enabled = bool(enabled)
		self.logger.debug("Raw payload logging %s", "enabled" if enabled else "disabled")
	...
	def get_ac_info(self):
		...
		if err == 0:
			...
			if self._raw_log_enabled:
				log_raw_payload("get_ac_info (decrypted)", response_payload)
			...
			self.logger.debug("Ambient Temp parsed: %s (byte15 raw: 0x%02X)" , ambient_temp, response_payload[15])
			
			if 0 <= ambient_temp <= 100:
				self.status['ambient_temp'] = ambient_temp
			else:
				self.logger.debug("Invalid ambient_temp: %s", ambient_temp)
			
			# Guess: room_hum at byte18(original)=response_payload[16], lower 7 bits
			raw_hum = response_payload[16] & 0x7F
			if 0 <= raw_hum <= 100:
				self.status['room_hum'] = raw_hum
			else:
				self.logger.debug("Invalid room_hum from byte16: %s (raw: 0x%02X)", raw_hum, response_payload[16])

			# err_code: default to 0 (no error) until we find the correct byte
			self.status['err_code'] = 0
			self.logger.debug("Parsed room_hum: %s", self.status.get('room_hum'))
	...
	def get_ac_states(self,force_update = False):    
		...
		if err == 0:
			aes = AES.new(bytes(self.key), AES.MODE_CBC, bytes(self.iv))
			response_payload_raw = bytes(aes.decrypt(bytes(response[0x38:])))
			response_payload = bytearray(response_payload_raw)
			...
			if self._raw_log_enabled:
				log_raw_payload("get_ac_states (decrypted)", response_payload)
			...
			self.logger.debug("Raw AC Status len=%d full: %s", len(response_payload), ' '.join(format(x, '02x') for x in response_payload))
	...
	def make_nice_status(self,status):
		...
		status_nice['room_hum'] = status.get('room_hum')
		status_nice['err_code'] = status.get('err_code', 0)
	...
	def get_key(self,list,search_value):
		...
		for key,value in list.items():
			if value == search_value:
				return key
		##Not found so return value as string to avoid calling .title() on int;
...
class ac_db_debug(device):
	...
	def get_ac_states(self,force_update = False):    
		...
		if err == 0:
			...
			self.status['turbo'] =response_payload[14] >> 6& 0b00000001
			self.status['clean'] = response_payload[18] >> 2& 0b00000001
			# Sensor data — only parse if payload is long enough
			if len(response_payload) > 16:
				self.status['room_hum'] = response_payload[16] & 0b01111111
			if len(response_payload) > 17:
				self.status['err_code'] = response_payload[17]
